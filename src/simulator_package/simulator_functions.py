...
class SimulationClass:
        ...
        for day in range(cm['time']):
            ...
            if self.analysis_mode:
                self.simulation["funnel_position_history"][:, day] = \
                    [self.compute_rescaled_funnel_position(cm, day, temp_usr) for temp_usr in range(cm['N_users'])]
    ...
    def save_result(self, cm, usr, day, has_converted):
        ...
        actions = self.user_list[usr]['actions'][initial_time:cm['time_failure']+initial_time]
        ...
        if has_converted:
            ...
            expositions = np.append(expositions, np.zeros([cm['N_camp'], cm['time_failure']+initial_time-day, 1]), axis=1)
